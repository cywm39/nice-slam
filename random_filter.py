...
def modify_depth_images(input_folder, output_folder, invalid_value):
    ...
    for file_name in file_list:
        ...
        # 计算要修改的像素数量
        num_modified_pixels = int(num_pixels * 0.3)
...
def modify_depth_images_Replica(input_folder, output_folder, invalid_value):
    # 创建输出文件夹
    os.makedirs(output_folder, exist_ok=True)
    ...
    for file_name in file_list:
        ...
        # 计算要修改的像素数量
        num_modified_pixels = int(num_pixels * 0.6)
...
# 指定输入文件夹和输出文件夹路径
input_folder = './Datasets/Replica/room0/results'
output_folder = './Datasets/Replica_depth_random_filter_60_3/room0/results'

# 调用函数进行深度图像修改和保存
modify_depth_images_Replica(input_folder, output_folder, invalid_value=0)
